...
class Lauta:
    ...
    def reset_play(self):
        ...
        actions = self.get_valid_moves2()

        return self.pelilauta2[np.random.choice(np.array(actions))]
    
    def simulated_human_player_step(self):

        actions = [i for i in range(len(self.pelilauta)) if self.pelilauta[i] in [0,4]]
        
        action = np.random.choice(actions)

        if self.pelilauta[action] == 4:
            self.pelilauta[action] = 5 # Pelaajan osuma agentin laivaan merkitään 5:llä
            self.hits_of_human += 1

        elif self.pelilauta[action] == 0:
            self.pelilauta[action] = 6  # Pelaajan hutilaukset merkitään 6:lla

        done = self.human_won()

        return done
    
    def simulated_human_player_step_play(self):

        actions = [i for i in range(len(self.pelilauta2)) if self.pelilauta2[i] in [0,4]]
        
        action = np.random.choice(actions)
        
        if self.pelilauta2[action] == 4:
            self.pelilauta2[action] = 5 # Pelaajan osuma agentin laivaan merkitään 5:llä
            self.hits_of_human_play += 1

        elif self.pelilauta2[action] == 0:
            self.pelilauta2[action] = 6  # Pelaajan hutilaukset merkitään 6:lla

        done = False
        if self.hits_of_human_play == MAX_OSUMAT:
            self.hits_of_human_play = 0
            self.hits_of_agent_play = 0
            done = True
    ...
    def human_won(self):
        ...
        if self.hits_of_human == MAX_OSUMAT:
            self.hits_of_human = 0
            self.hits_of_agent = 0
            return True
    ...
    def get_valid_moves(self):
        ...
        for i in range(len(self.pelilauta)):
            if self.pelilauta[i] in [1, 0]:
                actions.append(i)

        return actions

    def get_valid_moves2(self):
    ...
    def step(self, action):
        ...
        if self.pelilauta[action] == 1:
            self.hits_of_agent += 1
            self.pelilauta[action] = 2
            reward = 500.0
        
        # onko ammuttu hutilaukauskohtiin?
        elif self.pelilauta[action] == 3:
            reward = -200.0

        elif self.pelilauta[action] == 6:
            reward = -200.0

        # ei osumaa
        elif self.pelilauta[action] == 0:
            self.pelilauta[action] = 3
            reward = -250.0

        # Satunnaispelaajan kanssa joskus Q-taulun vielä kehittyessä
        # Q-agentti saattaa pelata satunnaispelaajan pussiin
        elif self.pelilauta[action] == 4:
            self.pelilauta[action] = 5
            self.hits_of_human += 1
            if self.hits_of_human == MAX_OSUMAT: human_won = True

            reward = -500.0
        elif self.pelilauta[action] == 5:
            reward = -500.0
        elif self.pelilauta[action] == 2:
        ...
        return self.pelilauta[action], reward, done, human_won
    
    def step_play(self, action):
        
        # osuma ihmispelaajan laivaan
        if self.pelilauta2[action] == 1:
            self.hits_of_agent_play += 1
        ...
        if self.hits_of_agent_play == MAX_OSUMAT:
            self.hits_of_agent_play = 0
            self.hits_of_human_play = 0
            done = True

        # havainto, palkkio, tieto onko peli pelattu läpi
        return self.pelilauta2[action], done